...
export function useCreateSlide(deckId: string) {
  return useMutation({
    mutationFn: async () => {
      const response = await requestJson<{ ok: true; slide: PersistedSlide }>(`/api/decks/${deckId}/slides`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export function useSaveSlide() {
  return useMutation({
    mutationFn: async (input: {
      ...
      const response = await requestJson<{ ok: true; slide: PersistedSlide }>(`/api/slides/${input.slideId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
...
export function useRollbackSlide() {
  return useMutation({
    mutationFn: async (input: { slideId: string; targetVersion: number; currentVersion: number }) => {
      const response = await requestJson<{ ok: true; slide: PersistedSlide }>(`/api/slides/${input.slideId}/rollback`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },

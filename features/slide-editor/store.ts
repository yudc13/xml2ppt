...
type SlideEditorState = {
	...
	cutSelectedShape: () => void
	pasteCopiedShape: (position?: { x: number; y: number }) => void
	...
	bringForward: () => void
	sendBackward: () => void
	alignSelectedShape: (alignment: 'left' | 'center' | 'right' | 'top' | 'middle' | 'bottom') => void
...
function reorderZIndex(
	...
	const index = sorted.findIndex((shape) => shape.id === selectedShapeId)
	if (index === -1) {
		return shapes
	}

	const selected = sorted[index]
	let reordered: EditableSlideShape[]

	if (mode === 'front') {
		const others = sorted.filter((_, i) => i !== index)
		reordered = [...others, selected]
	} else if (mode === 'back') {
		const others = sorted.filter((_, i) => i !== index)
		reordered = [selected, ...others]
	} else if (mode === 'forward') {
		if (index === sorted.length - 1) return shapes
		reordered = [...sorted]
...
function parseTableModel(contentNode: XmlValue | undefined): TableModel | null {
	...
	const rows = rowNodes
		.map((rowValue, rowIndex) => {
			...
			const rowId =
				typeof rowNode['@_id'] === 'string' ? rowNode['@_id'] : createId(`row-${rowIndex + 1}`)
...
export const useSlideEditorStore = create<SlideEditorState>((set, get) => ({
	...
	updateShapeSize: (shapeId, width, height) => {
		set((state) => ({
			shapes: updateShape(state.shapes, shapeId, (shape) => {
				...
				const maxWidth = isTextShape
					? Number.POSITIVE_INFINITY
					: SLIDE_WIDTH - shape.attributes.topLeftX
				const maxHeight = isTextShape
					? Number.POSITIVE_INFINITY
					: SLIDE_HEIGHT - shape.attributes.topLeftY
			...
			shapes: updateShape(state.shapes, shapeId, (shape) => {
				...
				return {
					...
					attributes: {
						...
						height: round2(
							clamp(
								Math.max(shape.attributes.height, rows.length * 44),
								MIN_SHAPE_SIZE,
								SLIDE_HEIGHT
							)
						),
			...
			shapes: updateShape(state.shapes, shapeId, (shape) => {
				...
				return {
					...
					attributes: {
						...
						width: round2(
							clamp(
								Math.max(shape.attributes.width, (rows[0]?.cells.length ?? 1) * 120),
								MIN_SHAPE_SIZE,
								SLIDE_WIDTH
							)
						),
	...
	cutSelectedShape: () => {
		set((state) => {
			...
			const snapshot = toHistorySnapshot(state)
			...
			return {
				...
				selectedShapeId: null,
				editingShapeId:
					state.editingShapeId === state.selectedShapeId ? null : state.editingShapeId,
				historyPast: [...state.historyPast, snapshot].slice(-HISTORY_LIMIT),
				historyFuture: [],
			}
		})
	},
	pasteCopiedShape: (position) => {
		set((state) => {
			const source =
			...
			if (position) {
				topLeftX = round2(clamp(position.x - width / 2, 0, SLIDE_WIDTH - width))
				topLeftY = round2(clamp(position.y - height / 2, 0, SLIDE_HEIGHT - height))
			} else {
				const offset = 24
				topLeftX = round2(clamp(source.attributes.topLeftX + offset, 0, SLIDE_WIDTH - width))
				topLeftY = round2(clamp(source.attributes.topLeftY + offset, 0, SLIDE_HEIGHT - height))
			}
			...
			const rawNode = structuredClone(source.rawNode)
			rawNode['@_id'] = id
			rawNode['@_topLeftX'] = topLeftX
			rawNode['@_topLeftY'] = topLeftY
			rawNode['@_zIndex'] = nextZIndex

			const pastedShape: EditableSlideShape = {
				...source,
				id,
				attributes,
				rawNode,
			...
			return {
				shapes: [...state.shapes, pastedShape],
				selectedShapeId: pastedShape.id,
				editingShapeId: pastedShape.attributes.type === 'table' ? pastedShape.id : null,
			}
		})
	},
	deleteSelectedShape: () => {
	...
	bringForward: () => {
		set((state) => {
			...
			return {
				shapes: reorderZIndex(state.shapes, state.selectedShapeId, 'forward'),
			}
		})
	},
	sendBackward: () => {
		set((state) => {
			...
			return {
				shapes: reorderZIndex(state.shapes, state.selectedShapeId, 'backward'),
			}
		})
	},
	alignSelectedShape: (alignment) => {
		set((state) => {
			if (!state.selectedShapeId) {
				return state
			}
			...
			const shapes = state.shapes.map((shape) => {
				...
				const { width, height } = shape.attributes
				let { topLeftX, topLeftY } = shape.attributes
				...
				switch (alignment) {
					case 'left':
						...
						break
					case 'center':
						topLeftX = (SLIDE_WIDTH - width) / 2
						break
					case 'right':
						topLeftX = SLIDE_WIDTH - width
						break
					case 'top':
						topLeftY = 0
						break
					case 'middle':
						topLeftY = (SLIDE_HEIGHT - height) / 2
						break
					case 'bottom':
						topLeftY = SLIDE_HEIGHT - height
						break
				}

				const nextTopLeftX = round2(topLeftX)
				const nextTopLeftY = round2(topLeftY)
				...
				return {
					...
					rawNode: {
						...shape.rawNode,
						'@_topLeftX': nextTopLeftX,
						'@_topLeftY': nextTopLeftY,
					},
				}
			})

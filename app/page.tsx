...
async function requestJson<T>(input: RequestInfo | URL, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok || payload.ok === false) {
    throw new ApiRequestError(payload.message ?? `Request failed: ${response.status}`, payload.code);
  }
...
export default function DeckListPage() {
  ...
  useEffect(() => {
    ...
    const loadDecks = async () => {
      ...
      setError(null);
      try {
        const response = await requestJson<{ ok: true; decks: Deck[] }>("/api/decks");
        if (!cancelled) {
  ...
  const handleCreateDeck = async () => {
    ...
    setError(null);
    try {
      const response = await requestJson<{ ok: true; deck: Deck }>("/api/decks", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          title: `未命名演示文稿 ${new Date().toLocaleDateString()}`,
        }),
      });

      router.push(`/decks/${response.deck.id}`);
    } catch (createError) {
